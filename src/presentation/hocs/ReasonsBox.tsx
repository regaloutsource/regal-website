...
const ReasonsBox: React.FC<IReasonBox> = ({ data, isReverse }) => {
  return (
    <Flex justify="space-around" align="center" className="reason-flex-box" style={{ backgroundImage: `${isReverse % 2 === 1 ? `linear-gradient(to right, #fff, #d4f7d4c6)` : `linear-gradient(to right, #d4f7d4c6, #fff)`}` }}>
        <Col span={10} style={{ order: isReverse % 2 === 0 ? 1 : 0 }}>
            <Image preview={false} className="reasons-img" src={data.image} alt={`${data.title}-img`} />
        </Col>
        <Col span={10}>
            <Typography.Paragraph   className="reasons-desp-text">{data.desp}</Typography.Paragraph >
        </Col>
    </Flex>
  )
}
